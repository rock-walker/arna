...
namespace AP.Server.Controllers
{
    ...
    public class OrderController : WorkshopTenantController
    {
        ...
        {
            Mapper.Initialize(cfg => cfg.CreateMap<OrderSeat, AssignSeat>());
        }
        */
        public OrderController(IWorkshopDao conferenceDao, IOrderDao orderDao, ICommandBus bus)
            : base(conferenceDao)
        {
        ...
        public ActionResult Display(Guid orderId)
        {
            ...
            if (order == null)
                return RedirectToAction("Find", new { conferenceCode = this.WorkshopCode });

            return View(order);
        }

        [HttpGet]
        public object AssignSeatsForOrder(Guid orderId)
        {
            var order = orderDao.FindPricedOrder(orderId);
        ...
        [HttpPost]
        public ActionResult AssignSeats(Guid assignmentsId, List<OrderAnchor> seats)
        {
            ...
            var pairs = seats
                ...
                .Select(seat => new { Saved = saved.Anchors.FirstOrDefault(x => x.Position == seat.Position), New = seat })
            ...
            var unassigned = pairs
                ...
                .Select(x => (ICommand)new UnassignAnchor { SeatAssignmentsId = saved.AssignmentsId, Position = x.Saved.Position });

            var changed = pairs
                .Where(x => x.Saved.Attendee != x.New.Attendee && x.New.Attendee.Email != null)
                .Select(x => (ICommand)Mapper.Map(x.New, new AssignAnchor { SeatAssignmentsId = saved.AssignmentsId }));
        ...
        public ActionResult Find(string attendee, string accessCode)
        {
            ...
            return RedirectToAction("Display", new { conferenceCode = this.WorkshopCode, orderId = orderId.Value });
        }
    }
}