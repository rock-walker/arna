...
namespace System.Threading.Tasks
{
	...
	public static class TaskExtensions
	{
		...
		public static Task<TResult> Catch<TResult>(this Task<TResult> task, Func<Exception, TResult> next)
		{
			if (task == null) throw new ArgumentNullException("task");
			if (next == null) throw new ArgumentNullException("next");
			...
			task.ContinueWith(delegate
			{
				if (task.IsFaulted)
				{
					try { tcs.TrySetResult(next(task.Exception)); }
					catch (Exception exc) { tcs.TrySetException(exc); }
				}
				else if (task.IsCanceled) tcs.TrySetCanceled();
		...
		{
			// assign a dummy result to task and pass to Catch<T>
			return task.Then(() => new object()).Catch(action);
		}
