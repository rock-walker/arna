...
namespace Infrastructure.Sql.EventSourcing
{
    ...
    public class SqlEventSourcedRepository<T> : AmbientContext<EventStoreDbContext>, IEventSourcedRepository<T> where T : class, IEventSourced
    {
        ...
        public SqlEventSourcedRepository(IEventBus eventBus, ITextSerializer serializer,
            IDbContextScopeFactory contextFactory, IAmbientDbContextLocator locator)
            : base(locator)
        {
            ...
            var constructor = typeof(T).GetTypeInfo().GetConstructor(new[] { typeof(Guid), typeof(IEnumerable<IVersionedEvent>) });
            if (constructor == null)
            {
        ...
        public T Find(Guid id)
        {
            ...
            {
                var deserialized = DbContext.Set<Event>()
        ...
        public void Save(T eventSourced, string correlationId)
        {
            ...
            using (var context = this.contextFactory.Create())
            {
                var eventsSet = DbContext.Set<Event>();
