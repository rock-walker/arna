namespace AP.Infrastructure.Sql.Processes
{
    using System;
    ...
    public class SqlProcessManagerDataContext<T> : IProcessManagerDataContext<T> where T : class, IProcessManager
    {
        ...
        public SqlProcessManagerDataContext(Func<DbContext> contextFactory, ICommandBus commandBus, ITextSerializer serializer, ILogger logger)
        {
        ...
        public T Find(Expression<Func<T, bool>> predicate, bool includeCompleted = false)
        {
            ...
            if (pm != null)
            {
                ...
                catch (DbUpdateConcurrencyException)
                {
                    ...
                    logger.LogWarning("Concurrency exception while marking commands as dispatched for process manager with ID {0} in Find method.", pm.ID);
        ...
        public void Save(T processManager)
        {
            ...
            {
                // if another thread already dispatched the messages, ignore
                logger.LogWarning("Ignoring concurrency exception while marking commands as dispatched for process manager with ID {0} in Save method.", processManager.ID);
            }
        }
